...
export default function Register() {
  ...
  const handleRegister = async (e) => {
    ...
    if (error) {
      ...
      alert("Berhasil daftar! Silakan masuk dengan akun baru Anda.");
      router.push("/login");
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex items-center justify-center p-6 selection:bg-blue-500/30">
      <div className="w-full max-w-md bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-xl shadow-blue-900/5 border border-slate-200 dark:border-slate-700">
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Buat Akun 🚀</h2>
          <p className="text-slate-500 dark:text-slate-400 mt-2">Daftar untuk mulai melaporkan</p>
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-5">
          ...
          <button
            ...
            className={`w-full py-3.5 rounded-xl font-bold text-white shadow-lg transition-all ${
              loading ? "bg-slate-400 dark:bg-slate-600 cursor-not-allowed" : "bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 hover:shadow-green-500/30 hover:-translate-y-0.5"
            }`}
          >
